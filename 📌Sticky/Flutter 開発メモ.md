...
- [x] プロトタイプ編集画面でチェーンを選択後、そのチェーンの詳細を見る（+編集する）導線が欲しい。UIの案を出して
	...
	- [ ] open_in_newで詳細からプロトタイプ編集画面を開いた時、保存ができない。内容を変更しても何も変更しなくても、「保存」を押すと「操作中に問題が発生しました。再度お試しください。」と表示される
	- [ ] プロトタイプ編集画面で、仮プロトタイプでない場合は「期日」の編集欄を非表示に
- [ ] 仮プロトタイプはいかなる場所で表示される時も頭に`[仮]`の文字を付けて。プロトタイプタブの一覧みたいに。
- [ ] プロトタイプ一覧画面の各業の右端に、今日のレーンに追加するボタンを追加。追加済の場合は可視化
- [ ] フォーカスドロワーから該当プロトタイプの編集画面をインラインで開けるように。
- [ ] オフ日設定と頻度設定の追加
