...
- [ ] " ログ反映前の変更確認モーダル ":  ログの反映処理を実行する前に、反映後の統計値と反映前との差分を示すモーダルを出し、ユーザーに確認＆承認させてから反映処理を進めます。モーダルは主要指標の一覧＋プロトタイプ別の詳細セクションを備え、変更後の値を強調表示しつつ差分を +/- で示すことで、変更の影響を即座に理解できる状態にします。UI の微調整は実装中に行います。確認後・キャンセル時、それぞれの遷移と状態管理も設計に含めます。
