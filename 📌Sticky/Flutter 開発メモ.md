...
- [ ] 「前日が未確定のためフォーカスを開始できません」のとき、ただsnackbarを出すだけでは不親切。改善案を出して。
- [ ] プロトタイプタブの「Add to Today / Added」機能について、フォーカス実行中、または実行済み（サマリーが存在する）のタスクについては「Added」の状態から変更できないようにしたい。まずは関係する文書を更新して。ただし少しでも疑問点がある場合、解決してから先に進むこと。
